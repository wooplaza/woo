				...
				return this.each(function() {
					...
					if (opt.get!="true" && opt.get!=true) {
						...
						if (window.tplogs==true)
							try{
								console.groupCollapsed("Essential Grid  1.2.1 Initialisation on "+container.attr('id'));
					...
					if (ival.length>0) {
						...
						jQuery.ajax({
						     ...
						     data:objData
								...
								//hidsbutton.click();
								jQuery(opt.filterGroupClass+'.esg-filter-wrapper .esg-allfilter').click();
			...
			************************************************/
			...
			container.on('itemsinposition',function() {
				...
				if (opt.maxheight>0 && opt.maxheight<9999999999) {
					...
					punchgs.TweenLite.to(esgo,heightspeed,{force3D:true,height:newheight,ease:punchgs.Power3.easeInOut,clearProps:"transform"});
				 ...
				 jQuery.each(esgAnimmatrix,function(index,key) {
					 item.find(key[0]).each(function() {
						 	 var elem = jQuery(this),
	  						  ...
	  						  if (art=="set" )
		  						punchgs.TweenLite.set(animobject,animfrom);
	  						  else
  						  		switch (key[0]) {
	  						  		case ".esg-shifttotop":
	  						  		...
	  						  		case ".esg-slide":
											...
											jQuery.extend(at,animto);
	  						  				af.css.x = xy.x;
	  						  				af.css.y = xy.y;
											punchgs.TweenLite.fromTo(animobject,key[1],af,at,elemdelay);
	  						  		break;
	  						  		case ".esg-slideout":
	...
	//	 	jQuery(this).wrapInner('<div class="eg-innerwrap-handlehideunder"></div>');
		 ...
		 ************************************************/
		 item.find('.eg-triggerfilter').click(function() {
			...
			jQuery(opt.filterGroupClass+'.esg-filterbutton,'+opt.filterGroupClass+' .esg-filterbutton').each(function() {
				if (jQuery(this).data('filter') == fil) jQuery(this).click();
			});
			return false;
		 }).css({cursor:"pointer"});
									  ...
									  if (maxdelay<dd) maxdelay = dd;
			  						  ...
			  						  switch (key[0]) {
			  						  		case ".esg-slide":
			  						  				...
			  						  				animto.x = xy.x;
			  						  				animto.y = xy.y;
													punchgs.TweenLite.to(animobject,0.5,{y:animto.y, x:animto.x,overwrite:"all",onCompleteParams:[animobject],onComplete:function(obj) {
			  						  		...
			  						  		case ".esg-slideout":
			  						  				...
			  						  				animto.y = 0;
			  						  				animto.overwrite = "all";
													punchgs.TweenLite.fromTo(animobject,0.5,{autoAlpha:1,x:xy.x,y:xy.y},{x:0,y:0,autoAlpha:1,overwrite:"all"});
			  						  		break;
...
/*jQuery("document").ready(function() {
	...
	var ajaxRevslider = function(obj) {
		...
		var content_is_loaded = false,
			content = "<h2>THIS IS SOME TITLE</h2><br/>";

        content += "Type:"+obj.type+"</br>";
		content += "ID:"+obj.id+"</br>";
		content += "Aspect Ratio:"+obj.aspectratio+"</br>";

		while ( content_is_loaded) {
			content_is_loaded=true;
		}
		return	content;	// FIRST RETURN THE CONTENT WHEN IT IS LOADED !!
	};

	// EXTEND THE AJAX CONTENT LOAING TYPES WITH TYPE AND FUNCTION
	var extendessential = setInterval(function() {
		if (jQuery.fn.tpessential != undefined) {
			jQuery.fn.tpessential.defaults.ajaxTypes.push({type:"revslider",func:ajaxRevslider});
