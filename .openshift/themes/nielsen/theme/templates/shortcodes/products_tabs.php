...
$delay = ( isset( $animation_delay ) && $animation_delay != '' ) ? 'data-delay="'. esc_attr( $animation_delay ) . '"' : '';

?>

<div class="tabs-container products_tabs <?php echo $animate ?>" <?php echo $delay ?>>
    <ul class="tabs">
        <?php for( $i = 1; isset( $atts['title_'.$i] ); $i++ ) :
            ...
            $sc[$yit_products_tabs_index] = '[products_slider title="' . $title . '" per_page="' . $per_page . '" featured="' . $featured . '" best_sellers="' . $best_sellers . '" on_sale="' . $on_sale . '" category="' . $category . '" orderby="' . $orderby . '" order="' . $order . '" layout="'. $layout .'" product_in_a_row="'. $product_in_a_row .'" ]';
