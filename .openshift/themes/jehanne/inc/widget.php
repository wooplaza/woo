 ...
 * Add a widget
 */
class jehanne_ExtendedWidget extends WP_Widget {

	function jehanne_ExtendedWidget() {

		/* Widget settings. */
		$widget_ops = array(
		'classname' => 'jehanne_extended',
		'description' => __('Display posts, pages, images, slideshow, news, related posts and more.', 'jehanne' ));

		/* Widget control settings. */
		$control_ops = array(
		'width' => 250,
		'height' => 250,
		'id_base' => 'jehanne_extended_widget');

		/* Create the widget. */
		$this->WP_Widget('jehanne_extended_widget', __( 'Jehanne: News, Related, Slideshow', 'jehanne' ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		if($instance['is_animate'] != '' && isset($instance['animate_style'])) {
			...
			$script = '<script type="text/javascript">
							...
							jehanne_animtype['.$str_id.'] = '. esc_js($instance['slider_style']).';
							jehanne_slidespeed['.$str_id.'] = '. esc_js($instance['slide_effect_speed']).';
							jehanne_timerinterval['.$str_id.'] = '. esc_js($instance['slide_speed']).';
							jehanne_textonslide['.$str_id.'] = '. esc_js($on_slide).';
		...
		if( $instance['is_show_custom_blocks'] == ''  ) {
			...
			foreach($posts as $currpost) { 
				...
				if( $instance['is_use_link_button'] != '' ){//add button
					...
					$buttont = '<br><a class="link-read-more" href="'.$link.'">'.$this->notEmpty(sanitize_text_field($instance['link_more_text']), __('Read more...', 'jehanne')).'</a>';
				}
			...
			for($i = 0; $i < $instance['number_of_posts']; $i++) { 
				...
				if( $instance['is_show_custom_link'] != '' ) {
					$buttont  = '<br><a class="link-read-more" href="'.esc_url($instance['link_'.$i]).'">'.$this->notEmpty(sanitize_text_field($instance['link_caption_'.$i]), __('Read more...', 'jehanne')).'</a>';
				}
				elseif( $instance['is_use_link_button'] != '' ){//add button
					$buttont = esc_url($instance['link_more_custom_link']);
					$buttont = '<br><a class="link-read-more" href="'.$buttont.'">'.$this->notEmpty(sanitize_text_field($instance['link_more_text']), __('Read more...', 'jehanne')).'</a>';
				}
	...
	function form( $instance ) {
	    ...
	    $this->echo_input_text('title', $instance, __( 'Title: ', 'jehanne' )); ?>
		<p>
		<?php $this->echo_input_text('number_of_posts', $instance, __( 'Number of posts: ', 'jehanne'), 0, 2); ?>
		</p>	
		
		<?php $this->echo_input_checkbox('is_show_custom_blocks', $instance, __( 'Display custom blocks', 'jehanne')); ?>

		<hr>
		<?php esc_html_e('Number of columns:', 'jehanne'); ?>
		...
		<hr>
		<p>
		<?php $this->echo_input_text('element_margin', $instance, __( 'Space Between Elements (%): ', 'jehanne'), 0, 2); ?>
		</p>

		<?php if($instance['is_show_custom_blocks'] == '') : ?>
			<p>
				<label for="<?php echo $this->get_field_id('cat_id'); ?>"><?php _e('Category ID:', 'jehanne'); ?></label>
				<?php wp_dropdown_categories('show_option_all='.__('any', 'jehanne').'&hide_empty=0&hierarchical=1&id='.$this->get_field_id('cat_id').'&name='.$this->get_field_name('cat_id').'&selected='.esc_attr($instance['cat_id'])); ?>
			</p>

			<p>
			<?php $this->echo_input_text('post_ids', $instance, __( 'Post Ids(the value for this field must be a list of integers, separated by commas):', 'jehanne'), 0, 15); ?>
			</p>
			<p>
			<?php $this->echo_input_text('page_ids', $instance, __( 'Page Ids(The value for this field must be a list of integers, separated by commas):', 'jehanne'), 0, 15); ?>
			</p>
			<?php $this->echo_input_checkbox('is_exclude_current_page_title', $instance, __( 'Exclude current post (Yes/No).', 'jehanne')); ?>
			<?php $this->echo_input_checkbox('is_use_current_category', $instance, __( 'Display recent posts from current category (Yes/No).', 'jehanne')); ?>

			<hr>
		<?php endif; ?>
		
		<label for="<?php echo $this->get_field_id('style'); ?>"><?php _e('Choose how to display an Image:', 'jehanne'); ?></label>
		<hr>
		<select id="<?php echo $this->get_field_id('style'); ?>" name="<?php echo $this->get_field_name('style'); ?>" style="width:100%;">
		<?php 
			$styles=array( __('Top', 'jehanne'), __('Right', 'jehanne'), __('Left', 'jehanne'), __('None', 'jehanne'));
			for ($i=0;$i<4;$i++) {
		...
		<hr>
		<?php $this->echo_input_text('image_size', $instance, __( 'Image size (%) for images with left and right alignment: ', 'jehanne'), 0, 2); ?>
		<?php $this->echo_input_checkbox('is_use_background_image', $instance, __( 'Display background Image(Yes/No)', 'jehanne')); ?>
		<?php $this->echo_input_checkbox('is_image_fixed', $instance, __( 'Fixed', 'jehanne')); ?>
		<?php $this->echo_input_checkbox('is_hide_on_small_screen', $instance, __( 'Hide on small screen(Yes/No)', 'jehanne')); ?>
		<?php $this->echo_input_upload('background_image', $instance, __( 'Upload background Image:', 'jehanne')); ?>
		<hr>
		<p>
		<?php $this->echo_input_checkbox('is_repeat_x', $instance, __( 'Repeat X', 'jehanne'), 0); ?>
		<?php $this->echo_input_checkbox('is_repeat_y', $instance, __( 'Repeat Y', 'jehanne'), 0); ?>
		</p>
		<hr>
		<?php esc_html_e('Display image on:', 'jehanne'); ?>
		<hr>
		<?php esc_html_e('X Position:', 'jehanne'); ?>
		<select id="<?php echo $this->get_field_id('pos_x'); ?>" name="<?php echo $this->get_field_name('pos_x'); ?>" style="width:100%;">
		<?php 
			$styles=array( __('Left', 'jehanne'), __('Right', 'jehanne'), __('Center', 'jehanne'));
		...
		<hr>
		<?php esc_html_e('Colors:', 'jehanne'); ?>
		<hr>
		
		<?php $this->echo_input_checkbox('is_use_background_color', $instance, __( 'Set Custom Colors(Yes/No)', 'jehanne')); ?>
		<?php $this->echo_input_color('background_color', $instance, __( 'Background:', 'jehanne'), '#ffffff'); ?>
		<?php $this->echo_input_color('main_title_color', $instance, __( 'Widget Title:', 'jehanne'), '#000'); ?>
		<?php $this->echo_input_color('title_color', $instance, __( 'Title:', 'jehanne'), '#118412'); ?>
		<?php $this->echo_input_color('text_color', $instance, __( 'Text:', 'jehanne'), '#757575'); ?>
		<?php $this->echo_input_color('link_color', $instance, __( 'Link:', 'jehanne'), '#d57b00'); ?>
		<hr>
		
		<?php esc_html_e('Font size:', 'jehanne'); ?>
		
		<hr>
		<p>
		<?php $this->echo_input_text('title_size', $instance, __( 'Title:', 'jehanne'), 0, 2); ?>
		<?php $this->echo_input_text('text_size', $instance, __( 'Text:', 'jehanne'), 0, 2); ?>
		<?php $this->echo_input_text('link_size', $instance, __( 'Link:', 'jehanne'), 0, 2); ?>
		</p>
		
		<?php if($instance['is_show_custom_blocks'] == '') : ?>
			<?php $this->echo_input_checkbox('is_show_title', $instance, __( 'Display the title', 'jehanne')); ?>
			
			<?php $this->echo_input_checkbox('is_show_content', $instance, __( 'Display the content', 'jehanne')); ?>
			<p>
			<?php $this->echo_input_text('content_length', $instance, __( 'Number of characters: ', 'jehanne'), 0, 4); ?>
			</p>
		<?php endif; ?>
		
		<?php if($instance['is_show_custom_blocks'] != '') :
			$this->echo_input_checkbox('is_show_custom_title', $instance, __( 'Display custom title', 'jehanne'));
			$this->echo_input_checkbox('is_show_custom_content', $instance, __( 'Display custom content', 'jehanne'));
			$this->echo_input_checkbox('is_show_custom_image', $instance, __( 'Display custom Image', 'jehanne'));
			$this->echo_input_checkbox('is_show_custom_link', $instance, __( 'Display custom Link', 'jehanne'));
			...
			for( $i = 0; $i < $instance['number_of_posts']; $i++) {
				...
				if( $instance['is_show_custom_image'])
					$this->echo_input_upload_id('image_'.$i, $instance, __( 'Image: ', 'jehanne' ), 0);
				if( $instance['is_show_custom_title'] )
					$this->echo_input_text('title_'.$i, $instance, __( 'Header: ', 'jehanne' ), 0);
				if( $instance['is_show_custom_content'] )
					$this->echo_input_textarea('text_'.$i, $instance, __( 'Text: ', 'jehanne' ), 2);
				if( $instance['is_show_custom_link'] ) {
					$this->echo_input_text('link_'.$i, $instance, __( 'Link: ', 'jehanne' ), 0);
					echo '<br>';
		...
		<?php esc_html_e('Border:', 'jehanne'); ?>
		
		<hr>
		<?php $this->echo_input_checkbox('is_use_border', $instance, __( 'Display an Image Border (Yes/No)', 'jehanne')); ?>
		<?php $this->echo_select_border_style('border_style', $instance); ?>
		<?php $this->echo_input_color('border_color', $instance, __( 'Border color:', 'jehanne'), '#ffffff'); ?>
		<p>
		<?php $this->echo_input_text('border_width', $instance, __( 'Border width:', 'jehanne'), 0, 2); ?>
		<?php $this->echo_input_text('border_radius', $instance, __( 'Border radius', 'jehanne'), 0, 2); ?>
		</p>
		<hr>		
		<?php $this->echo_input_checkbox('is_use_link_button', $instance, __( 'Display the "Read More" button (Yes/No)', 'jehanne'), '#ffffff'); ?>
		<hr>
		<?php $this->echo_input_color('link_more_background_color', $instance, __( 'Button background:', 'jehanne'), '#ffffff'); ?>
		<?php $this->echo_input_color('link_more_text_color', $instance, __( 'Button text color:', 'jehanne'), '#000000'); ?>
		<?php $this->echo_input_text('link_more_text_size', $instance, __( 'Button text size:', 'jehanne')); ?>
		<?php $this->echo_input_text('link_more_text', $instance, __( 'Button caption text:', 'jehanne')); ?>
		<hr>		
		<?php $this->echo_input_checkbox('is_use_link_more_custom_link', $instance, __( 'Display a custom link (Yes/No):', 'jehanne')); ?>
		<hr>
		<?php $this->echo_input_text('link_more_custom_link', $instance, __( 'Button URL:', 'jehanne')); ?>
		
		<?php $this->echo_select_border_style('link_more_border_style', $instance); ?>
		<?php $this->echo_input_color('link_more_border_color', $instance, __( 'Button border color:', 'jehanne'), '#000000'); ?>
		<hr>
		<?php $this->echo_input_text('link_more_border_width', $instance, __( 'Button border width:', 'jehanne'),1, 1); ?>
		<?php $this->echo_input_text('link_more_border_radius', $instance, __( 'Button border radius', 'jehanne'), 1, 2); ?>
		
		<hr>
		
		<?php esc_html_e('Margins:', 'jehanne'); ?>
		
		<p>
		<?php $this->echo_input_text('padding-top', $instance, __( 'Top:', 'jehanne'), 0, 2); ?>
		<?php $this->echo_input_text('padding-right', $instance, __( 'Right:', 'jehanne'), 0, 2); ?>
		</p>
		<p>
		<?php $this->echo_input_text('padding-bottom', $instance, __( 'Bottom:', 'jehanne'), 0, 2); ?>
		<?php $this->echo_input_text('padding-left', $instance, __( 'Left:', 'jehanne'), 0, 2); ?>
		</p>
		<hr>
		<hr>		
		<?php $this->echo_input_checkbox('is_use_icon', $instance, __( 'Display an Icon (Yes/No)', 'jehanne')); ?>
		<hr>
		<?php $this->echo_input_upload('icon', $instance, __( 'Upload an Image for the Icon:', 'jehanne')); ?>
		<p>
		<?php $this->echo_input_text('icon_h', $instance, __( 'Icon Height:', 'jehanne'), 0, 2); ?>
		<?php $this->echo_input_text('icon_w', $instance, __( 'Icon Width:', 'jehanne'), 0, 2); ?>
		</p>
		<hr>
		<?php esc_html_e('Animate:', 'jehanne'); ?>
		<hr>
		<?php $this->echo_input_checkbox('is_animate', $instance, __( '(Yes/No).', 'jehanne')); ?>
		<hr>
		
		<select id="<?php echo $this->get_field_id('animate_style'); ?>" name="<?php echo $this->get_field_name('animate_style'); ?>" style="width:100%;">
		<?php 
			$styles=array( __('Step', 'jehanne'), __('All', 'jehanne'));
		...
		<hr>
		<?php $this->echo_input_checkbox('is_slider', $instance, __( 'Slideshow(This option may override the options given in earlier fields)', 'jehanne')); ?>

		<?php $this->echo_input_checkbox('text_on_slide', $instance, __('Place Text over an Image(Yes/No).', 'jehanne')); ?>

		
		<select id="<?php echo $this->get_field_id('slider_style'); ?>" name="<?php echo $this->get_field_name('slider_style'); ?>" style="width:100%;">
		<?php 
			$styles=array( __('Fade In', 'jehanne'), __('Top', 'jehanne'), __('Right', 'jehanne'), __('Left', 'jehanne'));
		...
		<p>
		<?php $this->echo_input_text('slide_speed', $instance, __( 'Enter the desired transition time in milliseconds:', 'jehanne'), 0, 2); ?>
		</p>
		<p>
		<?php $this->echo_input_text('slide_effect_speed', $instance, __( 'Enter the desired effect time in milliseconds:', 'jehanne'), 0, 2); ?>
		</p>
		<?php $this->echo_input_color('background_text_color', $instance, __( 'Text Background:', 'jehanne'), '#ffffff'); ?>
	...
	function echo_select_border_style($name, $instance) { ?>
			<label for="<?php echo $this->get_field_id($name);?>"><?php esc_html_e('Border style:', 'jehanne'); ?></label>
			<hr>
	...
	function defaults( $instance ){
		...
		$defaults = array('title'=>'',
						...
						'link_more_text' => __( 'Read more...', 'jehanne'),
	...
	function defaults_for_count( $instance, $count ){
		...
		$defaults = array();
		for( $i = 0; $i < $count; $i++ ) {
			$defaults['title_'.$i] = __('Title', 'jehanne'); 
			$defaults['text_'.$i] = __('Description', 'jehanne'); 
			$defaults['link_'.$i] = ''; 
...
/* Register widget*/
function jehanne_register_widgets() {
	register_widget( 'jehanne_ExtendedWidget' );
}
add_action( 'widgets_init', 'jehanne_register_widgets' );
/*
* Hook widget css to wp_head
*/
function jehanne_hook_into_head_widget_css() {
	$css = '';
	$myWidget = new jehanne_ExtendedWidget();
	$widgets = $myWidget->get_settings();
			
	foreach ($widgets as $key => $instance) {
		$widget_id = 'jehanne_extended_widget-'.$key;
		$sidebar = is_active_widget( '', $widget_id, 'jehanne_extended_widget');
		if ( $sidebar ) {
			if($widgets[$key] != null) {
...
add_action('wp_head','jehanne_hook_into_head_widget_css');